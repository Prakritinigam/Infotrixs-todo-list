...
const Home = () => {
    ...
    const useremail=location.state.s_email;
...
const [inptodo, setTodo] = useState({ 
    ...
    console.log(
        getTaskData.filter((element) => element.tasks.includes(searchInput))
      );
    ...
    return (
        <>
            ...
            <div>
<br/><br/><br/>
<h3 className='all'>All Tasks</h3><br/> 
<center><input class="form-control " style={{"width":"300px"}} type="search" placeholder="Search your Task" onChange={(e) => setSearchInput(e.target.value)} value={searchInput}  aria-label="Search" /></center><br/>
                {
                    getTaskData.filter(
                        (element) => element.tasks.includes(searchInput)
                      )
                    .map((element, id) => {  
                        return (
                            <>
                            <div className='row show'>
                                ...
                                {element.tasks}    
                                      ...
                                      <div className='col-sm-2'> <Link to={`/edit/${element._id}`}> <button  className='space w3-hover-shadow'  ><i class="fa fa-edit"></i>&nbsp;&nbsp;&nbsp;Update</button></Link></div>
